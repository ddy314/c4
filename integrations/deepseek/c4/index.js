...
function durationFromEnv(name2, fallback, maximum) {
  const value = Number.parseInt(process.env[name2] ?? "", 10);
...
function activePolicy() {
  const name2 = process.env.C4_POLICY ?? "balanced";
  if (!Object.hasOwn(POLICIES, name2)) throw new Error(`Unknown C4_POLICY: ${name2}`);
  return POLICIES[name2];
}
...
function isProtectedPath(value) {
  ...
  const name2 = basename(value.replace(/^["'@]+|["']+$/g, ""));
  if (!name2 || PUBLIC_SUFFIX.test(name2)) return false;
  return /^\.env(?:\.|$)/i.test(name2) || /^(?:id_rsa|id_ed25519|credentials?(?:[._-]|$)|private[_-]?key(?:[._-]|$)|service[_-]?account(?:[._-]|$))/i.test(name2);
}
...
function canonicalToolName(name2) {
...
// integrations/deepseek/c4/index.source.mjs
var name = "dsh-plugin-c4";
var inject = ["tools"];
function apply(ctx) {

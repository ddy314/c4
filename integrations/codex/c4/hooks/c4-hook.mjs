...
function hardToolPolicy(tool, input2) {
...
async function inspectToolCall(tool, input2, client, { goal = "", signal } = {}) {
  const hard = hardToolPolicy(tool, input2);
  if (hard?.action === "block") {
    return { ...routeRisk({ probabilities: {}, hard, boundary: "tool" }), hard, observation: { boundary: "tool_call", probabilities: {}, hard }, probabilities: {}, inputHash: fingerprint(JSON.stringify({ tool, input: input2 })), usage: { inputTokens: 0, outputTokens: 0, costUsd: 0 }, latencyMs: 0 };
  }
  const state = { boundary: "before_tool_execution", task: redactSensitive(goal).slice(0, 1e3), tool, arguments: redactStructured(input2) };
  ...
  return { ...routed, hard, observation: { boundary: "tool_call", probabilities: { scope_violation: probabilities.scope_violation, destructive: probabilities.destructive }, hard }, probabilities, inputHash: fingerprint(JSON.stringify({ tool, input: input2 })), ...result };
}
...
function classifyFlowCall(tool, input2) {
  ...
  const target = String(input2?.path ?? input2?.file_path ?? input2?.filePath ?? "");
  const visibleContent = tool !== "bash" && ["content", "message", "text", "body"].find((key) => typeof input2?.[key] === "string");
  ...
  return {
    ...
    inlineContentHashes: visibleContent ? literalEvidenceHashes(input2[visibleContent]) : []
  };
}
...
// src/host-hooks.mjs
function errorDecision(host, event, input2 = {}) {
  if (event === "PreToolUse") {
  ...
  return host === "codex" ? { decision: "block", reason: "C4 result screening unavailable" } : { hookSpecificOutput: { hookEventName: "PostToolUse", updatedToolOutput: redactResult(input2.tool_response ?? {}) } };
}
...
async function handleHook(host, input2, guard = createGuard({ host, sessionId: input2?.session_id ?? input2?.sessionId })) {
  ...
  if (!tool) return errorDecision(host, event, input2);
  if (event === "PreToolUse") {
    const decision2 = await guard.toolCall(tool, input2.tool_input ?? {}, { goal: "" });
    if (decision2.action === "allow") return {};
    if (host === "claude" && decision2.action === "ask") {
      return { hookSpecificOutput: { hookEventName: event, permissionDecision: "ask", permissionDecisionReason: decision2.reason } };
    }
  ...
  return { hookSpecificOutput: {
    ...
    updatedToolOutput: redactResult(input2.tool_response ?? {}),
    additionalContext: decision.action === "ask" ? "C4 withheld a tool result pending review." : "C4 quarantined a tool result."
  } };
}
async function runHook(host, input2) {
  try {
    return await handleHook(host, input2);
  } catch {
    return errorDecision(host, input2?.hook_event_name, input2);
  }
}

// integrations/codex/c4/hooks/c4-hook.source.mjs
var chunks = [];
